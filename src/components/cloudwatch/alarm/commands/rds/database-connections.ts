...
export class DatabaseConnections extends CreateAlarmCommand {
    ...
    execute(parent?: AlarmStore) {
        ...
        const description = `Alert if ${metricName} of ${dbInstanceIdentifier} is greater than or equal to ${this.threshold} for ${this.extraConfigs?.datapointsToAlarm} datapoint(s) within ${this.extraConfigs?.evaluationPeriods} period(s) of ${this.extraConfigs?.period} seconds.`;
